...
def check_permission(user, method, permission_to):
    ...
    for group in groups:
        ...
        for permission in permissions:
            if permission.objects.filter(id=permission['permission_id'],codename=required_permission).exist():
                return True
                
class EmployeesPermission(permissions.BasePermission):
    menssage = 'Sem permissão de gerenciamento de funcionarios'
...
class GroupsPermission(permissions.BasePermission):
    menssage = 'Sem permissão de gerenciamento de grupos'
...
class GroupsPermissionsPermission(permissions.BasePermission):
    menssage = 'Sem permissão de gerenciamento de permissões'

    def has_permission(self, request, _view) -> bool | None:
        return check_permission(request.user, request.method, permission_to='permission')

class TaskPermission(permissions.BasePermission):
    menssage = 'Sem permissão de gerenciamento de tarefas'
